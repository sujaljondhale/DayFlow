...
async function getLeaveBalance(req, res) {
  try {
    ...
    approvedLeaves.forEach(leave => {
      const usedDays = parseInt(leave.used_days, 10) || 0;
      if (leave.leave_type === 'PAID') usedPaid = usedDays;
      if (leave.leave_type === 'SICK') usedSick = usedDays;
      if (leave.leave_type === 'UNPAID') usedUnpaid = usedDays;
    });
    return res.json({
      success: true,
      year: currentYear,
