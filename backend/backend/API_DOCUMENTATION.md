...
### `GET /leaves/balance`
Returns dynamic leave quota balances (Read from `leave_allocations` set by Admin).

- **Query Parameters**: `userId` (optional, defaults to logged-in user)
- **Response (`200 OK`)**:
```json
{
  "success": true,
...
### `GET /leaves/allocations` *(Admin/HR)*
Fetch dynamic leave allocations set for employees.

---

### `POST /leaves/allocations` *(Admin/HR Only)*
Dynamically allocate or adjust leave days for any employee.
...
{
  "success": true,
  "message": "Successfully allocated 30 days of PAID leave for employee #2",
  "allocation": {
    "userId": 2,
    "leaveType": "PAID",
    "allocatedDays": 30,
